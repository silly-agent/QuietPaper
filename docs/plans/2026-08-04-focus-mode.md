...
**Goal:** Add a toolbar button and `Command + E` shortcut that toggle a detail-only editing layout, with a centered paper column for Markdown files.

**Architecture:** Store the transient focus flag on `AppModel`, let `RootView` render either the editor alone or the normal `NavigationSplitView`, hide the window toolbar while focused, and route keyboard events through the existing notification-based shortcut bridge. `NoteEditorView` applies a roughly 760pt max-width paper surface and hides its title/header/status chrome while the Markdown editor is focused; request and connection editors remain separate RootView branches. The editor button binds to the same flag so visual state and layout state cannot diverge.
...
**Step 2a:** Verify a Markdown note uses a centered max-width paper surface with readable side margins and no editor chrome, while request and connection editors retain their existing layout.
